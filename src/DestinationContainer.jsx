  ...
  return (
    <div style={{display: 'grid', 
        ...
        {destinations.map((destination)=>{
            return <DestinationCard key={destination.id} name={destination.name} location={destination.location} image={destination.image} description={destination.description} price={destination.price}/>
        })}
    </div>
  )
}
